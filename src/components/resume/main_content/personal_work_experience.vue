<template>
  <div class="work_experience">
    <div style="padding-top: 15px" >
      ...
      <span style="font-size: 18px; color: #414a60">工作经历</span>
    ...
    <work_experience_edit_element v-if="new_id===-1" :is_new="true" @edit_finish_4="end_new" />
    ...
    <div v-for="id in id_list" style="padding-bottom: 20px;padding-right: 20px">
      <div class="element" :class="{po: move_on_id===id}" @mouseover="move_on(id)" @mouseout="move_out" >
        <work_experience_element v-if="!set_has(id)" :id="id" @click="edit(id)" />
        <work_experience_edit_element v-else :id="id" :is_new="false" @edit_finish_4="edit_finish(id)" />
      </div>
    </div>
...
<script>
import work_experience_element from "./experience/work_experience_element.vue";
import work_experience_edit_element from "./experience/work_experience_edit_element.vue";
import {reactive} from "vue";
import global from "../../global.js";

export default {
  name: "personal_work_experience",
  setup () {
  ...
  methods: {
    get_id_list() {
      // 从后端获取id_list
      let that = this
      that.$axios.post('people/get_my_work_list', {
        ...
        that.id_list = res.data.my_work_list
  ...
  components: {
    work_experience_element,
    work_experience_edit_element
  }
}
</script>
