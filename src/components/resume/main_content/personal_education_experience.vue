<template>
  <div class="work_experience">
    <div style="padding-top: 15px" >
      ...
      <span style="font-size: 18px; color: #414a60">教育经历</span>
    ...
    <education_experience_edit_element v-if="new_id===-1" :is_new="true" @edit_finish_6="end_new" />
    ...
    <div v-for="id in id_list" style="padding-bottom: 20px;padding-right: 20px">
      <div class="element" :class="{po: move_on_id===id}" @mouseover="move_on(id)" @mouseout="move_out" >
        <education_experience_element v-if="!set_has(id)" :id="id" @click="edit(id)" />
        <education_experience_edit_element v-else :id="id" :is_new="false" @edit_finish_6="edit_finish(id)" />
      </div>
    </div>
...
<script>
import education_experience_element from "./experience/education_experience_element.vue";
import education_experience_edit_element from "./experience/education_experience_edit_element.vue";
import {reactive} from "vue";
import global from "../../global.js";

export default {
  name: "personal_education_experience",
  setup () {
  ...
  methods: {
    get_id_list() {
      // 从后端获取id_list
      let that = this
      that.$axios.post('people/get_my_education_list', {
        ...
        that.id_list = res.data.my_education_list
  ...
  components: [
      education_experience_edit_element,
      education_experience_element
  ]
}
</script>
